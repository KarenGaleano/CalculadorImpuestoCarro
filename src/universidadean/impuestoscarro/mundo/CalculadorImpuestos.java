...
public class CalculadorImpuestos {
    ...
    private RangoImpuesto buscarRangoImpuesto(double valor) {
        ...
        for (int x = 0; (x < rangosImpuesto.length) && (rango == null); x++){
            RangoImpuesto rangoop = (RangoImpuesto) rangosImpuesto[x];//Creando rango OP
            if(rangoop.contieneA(valor))
                rango=rangoop;
        }
    ...
    public double calcularPago(boolean descProntoPago, boolean descServicioPublico, boolean descTrasladoCuenta) {
        ...
        if (descProntoPago) // Decuento por pronto pago
            pago -= pago * (PORC_DESC_PRONTO_PAGO / 100);
        if (descServicioPublico) //Restando descuento por servicio publico si aplica
            pago -= VALOR_DESC_SERVICIO_PUBLICO;
        if(descTrasladoCuenta) //Restando descuento por Traslado si aplica
            pago -= pago * (PORC_DESC_TRASLADO_CUENTA / 100);
        if (pago < 0) // Por si el pago queda en negativo despues de descuentos (por si las moscas)
            pago = 0;
        return pago;
    ...
    public Vehiculo buscarVehiculoMasCaro() {
        ...
        double caro= vehiculos[0].darPrecio();
    ...
    public Vehiculo buscarVehiculoMasAntiguo() {
        ...
        int comparar = 2021;
        for (Vehiculo v: vehiculos){
            int comparardos = Integer.parseInt(v.darAnio());
            if (comparardos < comparar){
                comparar =comparardos;
                masAntiguo = v;
            }
        }
        return masAntiguo;
    ...
    public double promedioPreciosVehiculos() {
        ...
        promedio = (int) suma / vehiculos.length;
        return promedio;
    }
